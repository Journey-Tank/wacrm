-- ============================================================
-- 041_broadcast_extended_stats.sql
--
-- Adds support for 'not_in_whatsapp' and 'frequency_limit' stats.
-- ============================================================

-- 1) Add new columns to broadcasts table
ALTER TABLE broadcasts ADD COLUMN IF NOT EXISTS not_in_whatsapp_count INTEGER DEFAULT 0;
ALTER TABLE broadcasts ADD COLUMN IF NOT EXISTS frequency_limit_count INTEGER DEFAULT 0;
...
ALTER TABLE broadcast_recipients ADD CONSTRAINT broadcast_recipients_status_check 
  CHECK (status IN ('pending', 'sent', 'delivered', 'read', 'replied', 'failed', 'not_in_whatsapp', 'frequency_limit'));
